...
public class BossBattle : MonoBehaviour
{
    ...
    void Update()
    {
        if (!battleEnded)
        {
            if (player != null)
            {
                Vector3 direction = player.position - theBoss.position;

                // Check if the player is to the left or right of the boss
                if (direction.x > 0)
                {
                    // Player is to the right, face right
                    theBoss.localScale = new Vector3(-1, 1, 1); // Ensure this matches your original scale
                }
                else
                {
                    // Player is to the left, face left
                    theBoss.localScale = new Vector3(1, 1, 1); // Flip along X-axis
                }
            }
            ...
            if (BossHealthController.instance.currentHealth > threshold1)
            {
                ...
                else if (inactiveCounter > 0)
                {
                    ...
                    {
                        theBoss.position = spawnPoints[UnityEngine.Random.Range(0, spawnPoints.Length)].position;
                ...
                else
                {
                    ...
                    else if (inactiveCounter > 0)
                    {
                        ...
                        if (inactiveCounter <= 0)
                        {
                            ...
                            targetPoint = spawnPoints[UnityEngine.Random.Range(0, spawnPoints.Length)];
                            int whileBreaker = 0;
